...
function buildPlanetsModel() {
  ...
  buttons.forEach(button => {
    ...
    const aptKey = getPlanetAptKey(sections, planetName);
...
function getPlanetAptKey(sections, planetName) {
  const ns = sections.galaxyMap?.querySelector('Namespace[Name="data"]');
  const ds = ns?.querySelector(`DataSource[Name="${planetName}"]`);
  ...
  return m ? m[1].toLowerCase() : planetName.toLowerCase();
}
...
// ADD PLANET
// ---------------------------------------------------------------------------
function addPlanet(displayName, key, aptTemplate, size) {
  if (!xmlDoc) return;
  ...
  if (ns) {
    ...
    ds.setAttribute('Name', key);
    const data = xmlDoc.createElement('Data');
    data.setAttribute('appearanceTemplate', aptTemplate || `appearance/ui_planet_sel_${key.toLowerCase()}.apt`);
  ...
  if (codeData) {
    codeData.setAttribute(`button${key}`, `map.GalaxyMap.button${key}`);
  }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("ap-displayName").oninput = e => {
    const key = e.target.value.replace(/\s+/g, '').replace(/[^A-Za-z0-9_]/g, '');
  ...
  document.getElementById("ap-confirmBtn").onclick = () => {
    const displayName = document.getElementById("ap-displayName").value.trim();
    ...
    closeAddPlanetModal();
    addPlanet(displayName, key, apt, size);
  };
